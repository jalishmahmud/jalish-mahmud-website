...
function publicPost(post) {
  ...
  const image = publicImage(post.coverImage, id, "cover", version) || post.image || siteUrl("/api/og/default");
  const seo = post.seo ? {
    ...post.seo,
    ogImage: publicImage(post.seo.ogImage, id, "og", version) || (/^https?:\/\//i.test(post.seo.ogImage || "") ? post.seo.ogImage : ""),
  } : post.seo;
