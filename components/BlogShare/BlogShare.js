...
export default function BlogShare({ title, url }) {
  ...
  return <div className={styles.share}><span>Share this article</span><div>{links.map(([label, href, Icon]) => <a key={label} href={href} target="_blank" rel="noreferrer" aria-label={`Share on ${label}`} onClick={(event) => openShare(event, href)}><Icon /></a>)}<button type="button" onClick={async () => { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1800); }}>{copied ? "Copied!" : "Copy link"}</button></div></div>;
}
